...
match_data <- country_chars %>%
  select(countrycode, ever_treated, mean_loggdp, mean_growth,
         mean_hc, mean_rkna, mean_pop, continent) %>%
...
love_plot <- love.plot(
  ...
  var.names    = c(
    mean_loggdp = "Log GDP per capita",
    mean_hc     = "Human Capital Index",
    continent   = "Continent"
  ),
...
plot_df_matched <- matched_data %>%
  ...
  select(countrycode, group, weights,
         mean_loggdp, mean_growth, mean_hc, mean_rkna, mean_pop) %>%
  pivot_longer(cols = starts_with("mean_"),
               names_to = "variable", values_to = "value") %>%
  mutate(variable = case_when(
    variable == "mean_loggdp" ~ "Log GDP per capita",
    variable == "mean_growth" ~ "GDP Growth",
    variable == "mean_hc"     ~ "Human Capital Index",
    variable == "mean_rkna"   ~ "Log Capital Stock",
    variable == "mean_pop"    ~ "Log Population"
  ))
...
p_density_matched <- ggplot(
  ...
  labs(
    ...
    subtitle = "Weighted by CEM matching weights",
    fill     = NULL,
    color    = NULL
  ) +
  theme_classic(base_size = 13) +
...
mod_balance <- feols(maxwind ~ loggdp + pop + rkna + hc,
                 data = data, vcov = ~countrycode)
mod_match <- feols(maxwind ~ loggdp + pop + rkna + hc,
...
map_data <- world %>%
  left_join(
    country_gdp %>% select(countrycode, income_quartile_label),
    by = c("adm0_a3" = "countrycode")
  )

fig_map <- ggplot(map_data) +
  geom_sf(aes(fill = income_quartile_label), color = "white", linewidth = 0.15) +
  scale_fill_manual(
    values = c(
      "Q1 (Poorest)" = "#d73027",
      "Q2"           = "#fc8d59",
  ...
  labs(
    title    = "Countries by Mean GDP per Capita Quartile",
...
ggsave("figures/map_income_quartiles.png", fig_map,
...
```{r}
# ------------------------------------------------------------------
# IRFs by income quartile using interaction LP
# ------------------------------------------------------------------

irfs_quartile <- lp_panel_inter(
  data = data,
  outcome = outcome,
  main_var = "maxwind_95",
  interact_var = "income_quartile",
  controls = make_controls("maxwind_95"),
  horizon = horizon,
  fe = fe,
  panel_id = panel_id,
  ...
  mutate(
    income_group = case_when(
      category == "1" ~ "Q1 (Poorest)",
      category == "2" ~ "Q2",
      category == "3" ~ "Q3",
      category == "4" ~ "Q4 (Richest)"
    )
  )
...
ggsave(
  "figures/irf_income_quartiles.png",
  fig_quartile,
  width = 14,
  height = 4.5,
  dpi = 300
...
irfs_quartile <- map_dfr(1:4, function(q) {
  label <- levels(country_gdp$income_quartile_label)[q]
  cat(sprintf("  Estimating LP for %s...\n", label))
  sub_data <- data_matched %>% filter(income_quartile == q)
...
ggsave("figures/irf_income_quartiles_by_period_csm.png", fig_quartile,
       width = 14, height = 4.5, dpi = 300)
...
```{r}
# ------------------------------------------------------------------
# IRFs by income quartile — original + matched
# ------------------------------------------------------------------
run_quartile_irfs <- function(df, label) {
  lp_panel_inter(
    data         = df,
    outcome      = outcome,
    main_var     = "maxwind_95",
...
irfs_quartile <- bind_rows(
  run_quartile_irfs(data,         "Unmatched"),
  run_quartile_irfs(data_matched, "Matched")
) %>%
  mutate(
    income_group = factor(
      income_group,
...
# ------------------------------------------------------------------
# Plot: 4-panel IRF figure with matched overlay
# ------------------------------------------------------------------
fig_quartile <- irfs_quartile %>%
  ggplot(aes(
    x     = horizon,
...
ggsave(
  "figures/irf_income_quartiles_matched.png",
  fig_quartile,
  width  = 14,
  height = 5,
  dpi    = 300
